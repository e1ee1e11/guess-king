...
class Hangman(object):
    ...
    def guess(self, letter):
        logging.debug("Guessing the letter " + letter + " in " + self.phrase)
        self.userGuess = letter.upper()
        self.usedLetters.append(self.userGuess)

        if self.inPhrase(self.userGuess) is False:
            self.numLives = self.numLives - 1

        for x in range(0, self.numChars):
            if self.phrase[x].upper() == self.userGuess.upper():
                self.underlinePhrase = self.underlinePhrase[:(2*x)] +\
                    self.phrase[x] + self.underlinePhrase[(2*x)+1:]
...
if __name__ == '__main__':
    ...
    while (True):
        ...
        print()
        logging.debug("Lives remaining: " + str(numLives))
        guessedLetter = input("Enter letter: ")
        for x in range(0, len(lettersUsed)):
            if guessedLetter.upper() == lettersUsed[x].upper():
                repeatLetter = True
                break
        if repeatLetter:
            logging.debug("This letter has been used already")
            continue
        else:
            hm.guess(guessedLetter)
        logging.debug(hm.getCurrentlyDiscoveredPhrase())
